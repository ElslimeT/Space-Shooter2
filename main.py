...
def main():
    ...
    while run:
        ...
        if len(enemies) == 0:
            level += 1
            wave_length += 5
            if player.max_health - player.health > 20:
                player.health += 20
            else:
                player.health = player.max_health
            for i in range(wave_length):
                enemy = Enemy(random.randrange(70, WIDTH-70), random.randrange(-500, -100), random.choice(["red", "green", "blue"]))
        ...
        if keys[pygame.K_w] and player.y - player_vel > 0: # up
            player.y -= player_vel
        if keys[pygame.K_s] and player.y + player.get_height() + player_vel + 20 < HEIGHT: # V
            player.y += player_vel
...
def main_menu():
    ...
    while run:
        WIN.blit(BG, (0, 0))
        title_lable = title_font.render("Press the mouse button to begin.", 1, (255, 255, 255))
        WIN.blit(title_lable, (WIDTH/2 - title_lable.get_width()/2, 300))
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
